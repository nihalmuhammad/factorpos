...
async function main() {
  ...
  try {
    ...
    console.log('\n2. Validation');
    {
      ...
      db.prepare("INSERT OR REPLACE INTO sequences (name, date, current_value) VALUES ('order_tokens', ?, 7)").run(todayLocal);
      ...
      const tokenSequence = db.prepare("SELECT current_value FROM sequences WHERE name = 'order_tokens' AND date = ?").get(todayLocal);
    ...
    console.log('\n15. POST /api/cash-closures/:id/print — Z print with forced drawer pulse (Wave 4, #649)');
    {
      ...
      try {
        ...
        const assertWidthBudget = (z: any, cols: number, tag: string): void => {
          ...
          assert(preview.includes('Generated by FactorPOS'), `${tag}: footer renders`);
        };
