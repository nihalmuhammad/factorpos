...
console.log('\n✅ Test 2: Compact receipt (80mm, 48 cols)');
{
  ...
  const doubleSizeBold = Buffer.from([ESC, 0x21, 0x38]);
  const tokenOffset = buf.indexOf(Buffer.from('*** TOKEN #42 ***'));
  const totalOffset = buf.indexOf(Buffer.from('TOTAL'));
  assert('prints token in double-size bold mode', tokenOffset > buf.indexOf(doubleSizeBold) && tokenOffset - buf.lastIndexOf(doubleSizeBold, tokenOffset) < 16);
...
console.log('\n✅ Test 11: IR country thermal receipt financial-line preservation & currency safety');
{
  ...
  const browserTokenHtml = generateBillHtml(tokenBill as any, frontendTenant, { useUnicode: false, template: 'compact' });
  ...
  const browserClassicHtml = generateBillHtml(tokenBill as any, frontendTenant, { useUnicode: false, template: 'classic' });
  assert('browser classic printing keeps the detailed item table', browserClassicHtml.includes('>Rate<') && !browserClassicHtml.includes('class="token"'));
