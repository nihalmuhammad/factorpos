...
export function generateBillHtml(
  ...
  const styles = getPaperStyles(paperSize, compact);
...
<body>
  <div class="bill-container">
    ...
    <div class="bill-details">
      ${compact && meta?.tokenNumber !== null && meta?.tokenNumber !== undefined ? `<div class="token">*** TOKEN #${escapeHtml(meta.tokenNumber)} ***</div>` : ''}
      <table>
        <tr>
    ...
    <table class="items-table">
      <thead>
        <tr>
          ...
          ${compact ? '' : `<th class="text-end">${escapeHtml(L.rate)}</th>`}
      ...
      <tbody>
        ${items.map(row => `
          <tr>
            ...
            ${compact ? '' : `<td class="text-end num">${fmtAmount(row.unitPrice ?? 0)}</td>`}
    ...
    <table class="totals-table">
      ...
      ${!compact || totals.discount || hasTax || totals.serviceCharge || totals.deliveryCharge || totals.packagingCharge ? `<tr><td>${escapeHtml(totals.subtotal.label.primary)}</td><td class="text-end num">${fmtAmount(totals.subtotal.amount)}</td></tr>` : ''}
    ...
    <table class="payments-table">
      ${compact ? '' : `<thead><tr><th colspan="2">${escapeHtml(L.paymentsHeader)}</th></tr></thead>`}
      <tbody>
...
function getPaperStyles(size: PaperSize, compact = false): string {
  const baseStyles = `
  ...
  const compactStyles = compact ? `
    ...
    .tax-table, .payments-table { width: 100%; margin-inline-start: 0; margin-bottom: 5px; }
    ...
    .total-row { font-size: 22px; line-height: 1.15; }
    .footer { margin-top: 8px; padding-top: 6px; }
    .footer p { margin: 0; }
    .powered-by { margin-top: 3px !important; }
  ` : '';
